<?php
	...
	$hari			= date("l",$day);
	$txt_text		= ["Submission - Annual Leave", "Notes", "Save", "Back", "Delete"];
	if($__group_id == "23"){
		$hari		= hari($day);
		$txt_text	= ["Pengajuan - Cuti Tahunan", "Keterangan", "Perbaharui", "Kembali", "Hapus"];
	}
	if(isset($_POST["save"])){
		$_errormessage 	= "";
		if($_errormessage == ""){
	...
	if($_GET["deleting"] && $data["	attendance_status"] != "27"){
		$db->addtable("attendance_activity");
		...
		<div class="col-lg-9 col-md-12 col-xs-12">
			<div class="post-job box">
				...
				<form class="form-ad" style="color: black;" enctype="multipart/form-data" method="POST" action="?token=<?=$token;?>&id=<?=$_GET["id"];?>">
					...
					&ensp;
					<a href="?token=<?=$token;?>&deleting=<?=$_GET["id"];?>&id=<?=$_GET["id"];?>" class="btn btn-danger"><?=$txt_text[4];?></a>
				</form>
			</div>
		</div>
	</div>
