<?php
	...
	$txt_text		= ["Submission - Special Leave", "Start Date", "To Date", "Notes", "Save", "Back"];
	if($__group_id == "23"){
		$hari		= hari($day);
		$txt_text	= ["Pengajuan - Cuti Khusus", "Mulai Tanggal", "Selesai Tanggal", "Keterangan", "Simpan", "Kembali"];
	}
	if(isset($_POST["save"])){
		$_errormessage 	= "";
		...
		if($_errormessage == ""){
			for($x=0; $x<$numdate; $x++){
				...
				$db->addfield("attendance_status"); 	$db->addvalue("29");
				$db->addfield("activity_id"); 			$db->addvalue("6");
