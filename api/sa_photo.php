...
<?php
	if($_GET["id"]){
		...
		$id 		= $db->fetch_single_data("indottech_sa","id",["site_id"=>$site_id, "created_by"=>$__username]);
	}
...
<center><h4><b>SA PHOTOS</b></h4></center>
<center><?=$_errormessage;?></center>
<table>
	<tr><td>SITE</td><td>:</td><td><?=$db->fetch_single_data("indottech_sites","name",["id" => $site_id]);?></td></tr>
</table>
<?php
	$back = $f->input("back","Back","type='button' onclick='window.location=\"sa_add.php?token=".$token."&id=".$id."&site_id=".$site_id."&mode=sa\";'","btn btn-warning");
	if($parent_id > 0){
...
<?php
	...
	$photos = $db->fetch_all_data("indottech_photo_items",[],"parent_id=".$parent_id." AND project_id=29 AND doctype LIKE 'sa_%'","seqno");
	foreach($photos as $photo){
		...
		if($is_parent > 0){
			$url_takephoto = "sa_photo.php?token=".$token."&id=".$id."&site_id=".$site_id."&photo_items_id=".$photo["id"]."&parent_id=".$photo["id"];
			$url_imagepick = "";
		} else {
			$url_takephoto = "sa_photo.php?token=".$token."&id=".$id."&site_id=".$site_id."&photo_items_id=".$photo["id"]."&parent_id=".$parent_id."&photo_name=".$photo["name"]."&site_name=".$site_name."&takephoto_offline=".$id."|".$photo["id"];
			$url_imagepick = "sa_photo.php?token=".$token."&id=".$id."&site_id=".$site_id."&photo_items_id=".$photo["id"]."&parent_id=".$parent_id."&photo_name=".$photo["name"]."&site_name=".$site_name."&imagepick=".$id."|".$photo["id"];
		}
?>
