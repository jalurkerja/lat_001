<?php
	...
	$indottech_plan_activities = $db->fetch_single_data("indottech_plan_activities","sow_ids",["id" => $_GET["plan_id"]]);
	if(!$_SESSION["sow_activity"]) {$_SESSION["sow_activity"] = pipetoarray($indottech_plan_activities);}
			...
			$sow_id_detail = $db->fetch_all_data($_tablename,[],"id ='".$sow_id."'")[0];
				...
				<tr>
					...
					if($_GET["mode"] == "wo_plan"){
						echo "<a href=\"?token=".$_GET["token"]."&plan_id=".$_GET["plan_id"]."&plan_site_id=".$_GET["plan_site_id"]."&remove=".$sow_id_detail["id"]."&mode=wo_plan\">Remove</a>";
					} else {
						echo "<a href=\"?token=".$_GET["token"]."&plan_id=".$_GET["plan_id"]."&plan_site_id=".$_GET["plan_site_id"]."&remove=".$sow_id_detail["id"]."\">Remove</a>";
					}
					?></td>
				</tr>
				<?php	
...
<?php
	if($_GET["plan_id"] == date("Ymd")){
		echo $f->input("close","Commit","type='button'  onclick=\"window.location='new_user_activity_add.php?token=".$_GET["token"]."';\"","btn btn-primary");
	} else if($_GET["mode"] == "wo_plan"){
		echo $f->input("close","Commit","type='button'  onclick=\"window.location='activity_wo_plan.php?token=".$_GET["token"]."';\"","btn btn-primary");
	} else {
		echo $f->input("close","Commit","type='button'  onclick=\"window.location='view_activities.php?token=".$_GET["token"]."&plan_id=".$_GET["plan_id"]."&plan_site_id=".$_GET["plan_site_id"]."';\"","btn btn-primary");
	}
?>
