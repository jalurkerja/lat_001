...
<table>
	<tr><td>SITE</td><td>:</td><td><?=$db->fetch_single_data("indottech_tag_photo_projects","site_name",["id" => $atd_id]);?></td></tr>
</table>
<?php
	$back = $f->input("back","Back","type='button' onclick='window.location=\"indosat_survey.php?token=".$token."\";'");
	$and_doctype = "AND doctype LIKE 'isat_survey_%' ";
	if($project_id == 3) {
		$back = $f->input("back","Back","type='button' onclick='window.location=\"h3i_photo_attachments.php?token=".$token."\";'");
		$and_doctype = "AND doctype LIKE 'h3i_atp_%' ";
	}
?>
<table width="100%"><tr>
	<td><?=$back;?></td>
...
<form method="POST" action="?token=<?=$token;?>&atd_id=<?=$atd_id;?>">
	<?php
		$photos = $db->fetch_all_data("indottech_photo_items",[],"parent_id=0 AND project_id='".$project_id."' ".$and_doctype,"seqno");
		foreach($photos as $photo){
			...
			if($is_parent > 0){
				$url_takephoto = "tag_photo_project_detail.php?token=".$token."&atd_id=".$atd_id."&photo_items_id=".$photo["id"];
				$url_imagepick = "";
			} else {
				$url_takephoto = "tag_photo_project_detail.php?token=".$token."&atd_id=".$atd_id."&photo_items_id=".$photo["id"]."&takephoto=".$atd_id."|".$photo["id"];
				$url_imagepick = "tag_photo_project_detail.php?token=".$token."&atd_id=".$atd_id."&photo_items_id=".$photo["id"]."&imagepick=".$atd_id."|".$photo["id"];
			}
	?>
