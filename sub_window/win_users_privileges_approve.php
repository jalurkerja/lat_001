<?php
	...
	if(@$_GET["remove"]){
		$_SESSION["users_privileges_approve"] = array_diff($_SESSION["users_privileges_approve"],array($_GET["remove"]));
	...
	if(@$_GET["add"]){
		if(in_array($_GET["add"],$_SESSION["users_privileges_approve"])) {
			$_title = "<p style='color:red; font-size:15px'>Please select other personil!</p>";
		} else {
			if(!$_SESSION["users_privileges_approve"]) {
				$_SESSION["users_privileges_approve"] = array($_GET["add"]);
			} else {
				$_SESSION["users_privileges_approve"] = array_merge($_SESSION["users_privileges_approve"],array($_GET["add"]));
			}
	...
	$user_values = "";
	$user = "";
	foreach($_SESSION["users_privileges_approve"] as $key_user => $user_id){
...
<h3><b>Teams Selected</b></h3>
	<table id="data_content">
		...
		<?php
			$personil_detail = "";
			foreach(@$_SESSION["users_privileges_approve"] as $no => $personil){
...
<?php
	...
	if(@$_SESSION["users_privileges_approve"]){
		$arr_add = "";
		foreach(@$_SESSION["users_privileges_approve"] as $arr){
			$arr_add .="id != ".$arr." AND ";
		}
