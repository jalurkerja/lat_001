<?php
	...
	if(@$_GET["remove"]){
		$_SESSION["users_privileges_delete"] = array_diff($_SESSION["users_privileges_delete"],array($_GET["remove"]));
	...
	if(@$_GET["add"]){
		if(in_array($_GET["add"],$_SESSION["users_privileges_delete"])) {
			$_title = "<p style='color:red; font-size:15px'>Please select other personil!</p>";
		} else {
			if(!$_SESSION["users_privileges_delete"]) {
				$_SESSION["users_privileges_delete"] = array($_GET["add"]);
			} else {
				$_SESSION["users_privileges_delete"] = array_merge($_SESSION["users_privileges_delete"],array($_GET["add"]));
			}
	...
	$user_values = "";
	$user = "";
	foreach(@$_SESSION["users_privileges_delete"] as $key_user => $user_id){
...
<h3><b>Teams Selected</b></h3>
	<table id="data_content">
		...
		<?php
			foreach(@$_SESSION["users_privileges_delete"] as $no => $personil){
...
<?php
	...
	if($_SESSION["users_privileges_delete"]){
		$arr_add = "";
		foreach($_SESSION["users_privileges_delete"] as $arr){
			$arr_add .="id != ".$arr." AND ";
		}
