<?php
	...
	if(@$_GET["remove"]){
		$_SESSION["users_privileges_view"] = array_diff($_SESSION["users_privileges_view"],array($_GET["remove"]));
	...
	if(@$_GET["add"]){
		if(in_array($_GET["add"],$_SESSION["users_privileges_view"])) {
			$_title = "<p style='color:red; font-size:15px'>Please select other personil!</p>";
		} else {
			if(!$_SESSION["users_privileges_view"]) {
				$_SESSION["users_privileges_view"] = array($_GET["add"]);
			} else {
				$_SESSION["users_privileges_view"] = array_merge($_SESSION["users_privileges_view"],array($_GET["add"]));
			}
	...
	$user_ids = "";
	$user_values = "";
	foreach($_SESSION["users_privileges_view"] as $key_user => $user_id){
...
<h3><b>Teams Selected</b></h3>
	<table id="data_content">
		...
		<?php
			foreach(@$_SESSION["users_privileges_view"] as $no => $personil){
...
<?php
	...
	if($_SESSION["users_privileges_view"]){
		$arr_add = "";
		foreach($_SESSION["users_privileges_view"] as $arr){
			$arr_add .="id != ".$arr." AND ";
		}
